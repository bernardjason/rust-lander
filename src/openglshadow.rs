...
impl OpenglShadow {
    ...
    pub fn new(gl: &gl::Gl) -> OpenglShadow {
        ...
        let (depth_map_fbo, depth_map) = {
            ...
            unsafe {
                ...
                // WHAT IS THIS?????
                //gl.TexParameterfv(gl::TEXTURE_2D, gl::TEXTURE_BORDER_COLOR, vec4(1.0f32,1.0,1.0,1.0).as_ptr());
    ...
    pub fn before_renderscenenormal(&mut self, gl: &gl::Gl, camera:Vector3<f32>) {
        unsafe {
            ...
            gl.ActiveTexture(gl::TEXTURE1); //        glActiveTexture(GL_TEXTURE1);
            gl.BindTexture(gl::TEXTURE_2D, self.depth_map); //        glBindTexture(GL_TEXTURE_2D, depthMap);
        }
    }
}
